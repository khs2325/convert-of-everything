...
| ReSprite `.resprite` | Supported subset | Preserves supported normal raster layer names, bottom-to-top order, visibility, opacity, signed cel placement, concrete and inherited RGBA cels, and supported clip tags. | Rebuilds ordered frames and converts frame-rate duration units to normalized whole milliseconds. | Accepts the [documented ReSprite bundle subset](resprite-format.md) with matching `document.json` and `meta.json` plus PNG-backed `CellData`. Groups, clipping masks, non-normal blend modes, per-cel opacity, palettes, tilemaps, reference images, editor metadata, ZIP64, and encrypted archives are unsupported. |
